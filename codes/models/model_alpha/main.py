...
states, _ = preprocessing_from_matlab(states_raw)
forces, _ = preprocessing_from_matlab(forces_raw)

# Standard format for Torch dataset
dataset = StatesAndForces(states, forces)
...
train_data, test_data = torch.utils.data.random_split(
    dataset, [TRAIN_PROP, 1 - TRAIN_PROP]
)
...
with alive_bar(
    ...
    for epoch in range(epochs):
        ...
        bar.text(f"| Train loss: {loss:.8f} | Test loss: {test_loss:.8f}")
        loss_overall.append(loss.item())
        loss_overall_test.append(test_loss.item())
        bar()


plot_loss_function(epochs=EPOCHS, loss=loss_overall, loss_test=loss_overall_test, n=1)
...
path_saved = os.path.join(path_to_model, "model.pt")
