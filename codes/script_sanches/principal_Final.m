...
% xs_all = cell(nnn,1);
  ...
  for loop=1:2
    ...
    option = 1;         % 1 = retangular  2 = circular  e 3 = linear        % Escolher a trajet�ria desejada

    x_des(1)=0;y_des(1)=0;z_des(1)=0; x_dot_des(1)=0;y_dot_des(1)=0;z_dot_des(1)=0;     % Condi��o inicial das trajet�rias
    for ii=1:length(t)
    [x_des0,y_des0,z_des0,x_dot_des0,y_dot_des0,z_dot_des0]=trajetoria(t(ii),option);                        % Trajet�rias desejadas
 ...
 % Funcao nftool
    ...
    xs_all{loop} = xs;
    disp(loop)
    % save("tau_all_large.mat", "tau_all")
    % save("xs_all_large.mat", "xs_all")
