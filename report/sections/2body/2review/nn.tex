...
\begin{figure}[!htb]
    ...
    \caption[Loss Function for Linear Regression]{Loss Function for Linear Regression. The loss function take all the distances between the predicted and the target value to verify if the model is in the right path. The lower the distance, the better.}
    \label{fig:mae_chart}
\end{figure}

For regression problems, common loss functions adopted are the \gls*{mse} \citep{bussab2017} or \gls*{mae}. 
%
\begin{align}
...
There are several ways that the data can be classified like multi-classification or binary classification. For multi-classification, the cross-entropy function is a common loss function adopted.
...
As the~\cref{eq:cross_entropy} is used for muti-classification, the binary cross-entropy, as the name suggest, is used to binary classification. It is defined as
%
\begin{equation}
