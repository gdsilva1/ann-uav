...
\gls*{ann} are the heart of \gls*{dl} and there are several implementationS of them for different kind of problems.
...
%
where 
\begin{description}[labelsep=0.5cm]
    \item[\(n\)] is the sample size; 
    \item[\(y_i\)] is the predicted output; and
    \item[\(\hat{y}_i\)] is the real target.
\end{description}
...
where 
\begin{description}[labelsep=0.5cm]
    \item[\(Q\)] is a discrete distribution relative to another distribution;
    \item[\(P\)] is the another discrete distribution;
...
%
where 
\begin{description}[labelsep=3cm]
...
Explain the Adam algorithm based on the \citet{kingma2017} article.