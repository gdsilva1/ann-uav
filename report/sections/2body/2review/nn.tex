...
A convolutional network is a \gls*{mlp} specifically projected to recognize two-dimensional shapes with a high-level invariance relative to translation, scaling, skew, and other forms of distortion. This task is learned in a supervised way, through these restrictions:
%
\begin{itemize}
...
\begin{figure}[!htb]\scriptsize
    ...
    \includesvg{figures/3review/nn/cnn.svg}
    \begin{flushleft}\footnotesize
        Source: prepared by the author.
    \end{flushleft}
\end{figure}
...
% \begin{tcolorbox}[title={\textbf{Adam Algorithm.} Good default setting are \(\alpha = 0.001,\ \beta_1 = 0.9,\ \beta_2 = 0.999\ \text{and}\ \epsilon = 10^{-8}\). Operations on vectors are element-wise.}]
...
% \State \(m_0 \gets 0\) \Comment{Initialize first moment vector}
% \State \(v_0 \gets 0\) \Comment{Initialize second moment vector}
% \State \(t \gets 0\) \Comment{Initialize timesep}

% \While{\(\theta_t\) not converged}
%     \State{\(t \gets t + 1\)}
%     \State{\(g_t \gets \nabla_{\theta}f_t(\theta_{t-1})\)} \Comment{Get gradients w.r.t. stochastic objective at time step \(t\)}
%     \State{\(m_t \gets b_1 \cdot m_{t-1} + (1-\beta_1) \cdot g_t\)} \Comment{Update biased first moment estimate}
%     \State{\(v_t \gets \beta_2\cdot v_{t-1} + (1-\beta_2)\cdot g_t^2\)} \Comment{Update biased second raw moment estimate}
%     \State{\(\widehat{m}_t \gets m_t/(1-\beta_1^t)\)} \Comment{Compute bias-corrected first moment estimate}
%     \State{\(\widehat{v}_t \gets v_t/(1-\beta_2^t)\)} \Comment{Compute bias-corrected second raw moment estimate}
%     \State{\(\theta_t \gets \theta_{t-1} - \alpha\cdot\widehat{m}_t/(\sqrt{\widehat{v}_t + \epsilon})\)} \Comment{Update parameters}
% \EndWhile

% \noindent \Return \(\theta_t\) \Comment{Resulting parameters}
% \end{algorithmic}
% \end{tcolorbox}
% Good default setting are \(\alpha = 0.001,\ \beta_1 = 0.9,\ \beta_2 = 0.999\ \text{and}\ \epsilon = 10^{-8}\). Operations on vectors are element-wise.